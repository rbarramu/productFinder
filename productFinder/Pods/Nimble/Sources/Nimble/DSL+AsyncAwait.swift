...
public func expecta(file: FileString = #file, line: UInt = #line, _ expression: @autoclosure () -> (() async throws -> Void)) async -> AsyncExpectation<Void> {
    ...
    public func waitUntil(timeout: NimbleTimeInterval = PollingDefaults.timeout, file _: FileString = #file, line _: UInt = #line, action: @escaping (@escaping () -> Void) async -> Void) async {
    ...
    private func throwableUntil(
        ...
        line: UInt = #line,
        action: @escaping (@escaping () -> Void) async throws -> Void
    ) async {
        ...
        let result = await performBlock(
            ...
            leeway: leeway,
            file: file, line: line
        ) { @MainActor(done: @escaping (ErrorResult) -> Void) async throws -> Void in
            do {
                try await action {
                    done(.none)
                }
