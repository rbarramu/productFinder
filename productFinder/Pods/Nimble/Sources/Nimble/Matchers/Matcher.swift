...
public extension Matcher {
    ...
    static func define(matcher: @escaping (Expression<T>) throws -> MatcherResult) -> Matcher<T> {
        Matcher<T> { actual in
            try matcher(actual)
        }.requireNonNil
    }
    ...
    static func define(_ message: String = "match", matcher: @escaping (Expression<T>, ExpectationMessage) throws -> MatcherResult) -> Matcher<T> {
        Matcher<T> { actual in
    ...
    static func defineNilable(_ message: String = "match", matcher: @escaping (Expression<T>, ExpectationMessage) throws -> MatcherResult) -> Matcher<T> {
        Matcher<T> { actual in
            try matcher(actual, .expectedActualValueTo(message))
        }
    }
...
public extension Matcher {
    ...
    ///
    /// Also ensures the matcher's actual value cannot pass with `nil` given.
    static func simple(_ message: String = "match", matcher: @escaping (Expression<T>) throws -> MatcherStatus) -> Matcher<T> {
        Matcher<T> { actual in
            MatcherResult(status: try matcher(actual), message: .expectedActualValueTo(message))
        }.requireNonNil
    }
    ...
    static func simpleNilable(_ message: String = "match", matcher: @escaping (Expression<T>) throws -> MatcherStatus) -> Matcher<T> {
        Matcher<T> { actual in
            MatcherResult(status: try matcher(actual), message: .expectedActualValueTo(message))
        }
    }
...
public struct MatcherResult {
    ...
    public init(bool: Bool, message: ExpectationMessage) {
        status = MatcherStatus(bool: bool)
        self.message = message
    }
...
extension Matcher {
    ...
    internal func after(f: @escaping (Expression<T>, MatcherResult) throws -> MatcherResult) -> Matcher<T> {
        ...
        Matcher { actual -> MatcherResult in
    ...
    public final class NMBMatcherResult: NSObject {
        ...
        public func toSwift() -> MatcherResult {
            MatcherResult(status: status.toSwift(),
    ...
    public extension MatcherResult {
        func toObjectiveC() -> NMBMatcherResult {
            NMBMatcherResult(status: status.toObjectiveC(), message: message.toObjectiveC())
        }
    }

    public final class NMBMatcherStatus: NSObject {
        private let status: Int
        private init(status: Int) {
            self.status = status
        }

        public static let matches = NMBMatcherStatus(status: 0)
        public static let doesNotMatch = NMBMatcherStatus(status: 1)
        public static let fail = NMBMatcherStatus(status: 2)
        ...
        public static func from(bool success: Bool) -> NMBMatcherStatus {
            self.from(status: MatcherStatus(bool: success))
        }
    ...
    public extension MatcherStatus {
        func toObjectiveC() -> NMBMatcherStatus {
            NMBMatcherStatus.from(status: self)
        }
    }
