...
public func beLessThanOrEqualTo<T: Comparable>(_ expectedValue: T?) -> Matcher<T> {
    Matcher.simple("be less than or equal to <\(stringify(expectedValue))>") { actualExpression in
...
public func <= <T: Comparable>(lhs: AsyncExpectation<T>, rhs: T) async {
    ...
    public extension NMBMatcher {
        @objc class func beLessThanOrEqualToMatcher(_ expected: NMBComparable?) -> NMBMatcher {
