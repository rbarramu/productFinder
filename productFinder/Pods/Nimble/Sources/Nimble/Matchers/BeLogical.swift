...
public func beTruthy<T: ExpressibleByBooleanLiteral & Equatable>() -> Matcher<T> {
    Matcher.simpleNilable("be truthy") { actualExpression in
...
public func beFalsy<T: ExpressibleByBooleanLiteral & Equatable>() -> Matcher<T> {
    Matcher.simpleNilable("be falsy") { actualExpression in
        let actualValue = try actualExpression.evaluate()
        return MatcherStatus(bool: actualValue != (true as T))
    }
}
    ...
    public extension NMBMatcher {
        @objc class func beTruthyMatcher() -> NMBMatcher {
        ...
        @objc class func beFalsyMatcher() -> NMBMatcher {
            NMBMatcher { actualExpression in
                let expr = actualExpression.cast { ($0 as? NSNumber)?.boolValue ?? false }
        ...
        @objc class func beTrueMatcher() -> NMBMatcher {
            NMBMatcher { actualExpression in
                let expr = actualExpression.cast { ($0 as? NSNumber)?.boolValue ?? false }
        ...
        @objc class func beFalseMatcher() -> NMBMatcher {
            NMBMatcher { actualExpression in
