    ...
    internal enum AsyncPollResult<T> {
        ...
        func toPollResult() -> PollResult<T> {
            ...
            case let .completed(value): return .completed(value)
            case let .errorThrown(error): return .errorThrown(error)
            }
        }
    }
    ...
    private func timeout<T>(timeoutQueue: DispatchQueue, timeoutInterval: NimbleTimeInterval, forcefullyAbortTimeout _: NimbleTimeInterval) async -> AsyncPollResult<T> {
        do {
